...
class VideoConverter {
    ...
    func convertVideoToHEIF(from videoURL: URL) async throws -> (photoData: Data, videoURL: URL) {
        ...
        // 添加音频相关元数据
        let hasAudioTrack = audioTrackForReaderOpt != nil
        print("VideoConverter: Audio track present: \(hasAudioTrack)")
        
        if let audioMetadata = createAudioMetadata() {
            metadata.append(audioMetadata)
            print("VideoConverter: Added audio metadata to Live Photo")
        }
