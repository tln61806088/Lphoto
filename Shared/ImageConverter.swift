...
class ImageConverter {
    ...
    func convertImageToHEIF(from imageURL: URL) async throws -> URL {
        ...
        guard let image = UIImage(data: imageData) else {
            throw NSError(domain: "ImageConverter", code: -1, userInfo: [NSLocalizedDescriptionKey: "无法读取图片数据"])
        }
        ...
        guard let heifData = image.heifData(compressionQuality: 0.8) else {
            throw NSError(domain: "ImageConverter", code: -1, userInfo: [NSLocalizedDescriptionKey: "无法转换图片为HEIF格式"])
        }
